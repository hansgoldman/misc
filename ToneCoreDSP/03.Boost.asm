...
;---------------------------------------------------------------------------------------------------
; 03.Boost.asm
;   This program is a simple pedal which uses a knob to control the overall level of the signal.
;
...
esai_rxeven_isr
    ...
    ; Process Left Channel (left input sample is in a)
        move    x:Knob_1,x0             ; load Knob 1 current value (range = 0 to 1)
        move    a,y0                    ; Move spample into y0
        mpy     x0,y0,a                 ; a = knob_1 * sample
        asl     #3,a,a                  ; Shift bits to the left 3 places
    ...
    ; Process Right Channel (r6 is currently pointing to Switch 1)
        ...
        ; Repeat same steps for Right channel
        move    x:Knob_1,x0             ; load Knob 1 current value (range = 0 to 1)
        move    a,y0                    ; Move spample into y0
        mpy     x0,y0,a                 ; a = knob_1 * sample
        asl     #3,a,a                  ; Shift bits to the left 3 places
