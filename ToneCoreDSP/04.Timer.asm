...
;---------------------------------------------------------------------------------------------------
; Tutorial 04 - Timer.asm
;   This is a tutorial to show how to build a timer for time-based effects.
;
; Written by Hans Goldman - Jan 2013
;---------------------------------------------------------------------------------------------------
...
TimerCounter            ds  1       ;       used to create "time" cycles
TimerFlag               ds  1       ;       Keeps track of timer cycles
TIMER_MAX_VALUE         equ $00AFFF ;       Max value of counter.

a0_backup               ds  1       ;       Backup for a0
a1_backup               ds  1       ;       Backup for a1
b0_backup               ds  1       ;       Backup for b0
b1_backup               ds  1       ;       Backup for b1
...
decrementTimer
        move    a0,x:a0_backup              ; Save register's value
        move    a1,x:a1_backup
        move    b0,x:b0_backup
        move    b1,x:b1_backup

        move    x:TimerCounter,a
        move    a,x:Debug_Read_from_DSP_1   ; send timer value to Debug Var
        cmp     #0,a                        ; check if TimerCounter is 0
        beq     timerIsZero                 ; if TimerCounter is 0, branch to timerIsZero
        move    x:TimerCounter,a
        dec     a                           ; decrement if not 0
        move    a,x:TimerCounter
        bra     exitTimer                   ; branch to exitTimer

timerIsZero
        move    #TIMER_MAX_VALUE,a          ; reset timerCounter to max value
        move    a,x:TimerCounter
        move    x:TimerFlag,a
        bchg    #0,a                        ; flip 0th bit of TimerFlag
        move    a,x:Debug_Read_from_DSP_2   ; send flag value to Debug Var
        move    a,x:TimerFlag               ; save value in TimerFlag

exitTimer
        move    x:a0_backup,a0              ; restore register's value
        move    x:a1_backup,a1
        move    x:b0_backup,b0
        move    x:b1_backup,b1
